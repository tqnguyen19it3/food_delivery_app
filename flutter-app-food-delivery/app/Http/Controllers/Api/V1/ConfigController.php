...
class ConfigController extends Controller
{
        public function geocode_api(Request $request)
    {
        ...
        $response = Http::get('https://maps.googleapis.com/maps/api/geocode/json?latlng='.$request->lat.','.$request->lng.'&key='."AIzaSyBBMeIKiDrIhzuSfwEWzLt0nkWZU7g7jOM");
    ...
    public function place_api_autocomplete(Request $request){
        ...
        if($validator->errors()->count() > 0){
            return response()->json(
                ['errors' => Helpers::errors_processor($validator)], 403
            );
        }
        $response = Http::get(
            ...
            .$request['search_text']
            .'&key='
            .'AIzaSyBBMeIKiDrIhzuSfwEWzLt0nkWZU7g7jOM'
        );
        return $response->json();
    }
